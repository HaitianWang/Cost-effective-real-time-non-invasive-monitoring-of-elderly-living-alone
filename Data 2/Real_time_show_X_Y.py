...
# 初始化图形
fig, ax = plt.subplots()
x_data, y_data, time_data = deque(maxlen=1000), deque(maxlen=1000), deque(maxlen=1000)
...
def init():
    ...
    ax.set_ylim(-2, 2)  # 假定加速度范围为±2m/s^2
...
def update(frame):
    try:
        ...
        if data:
            ...
            # 显示包编号和其他信息
            print(f"Packet {packet_number}: X={x}, Y={y}, State={state}")
